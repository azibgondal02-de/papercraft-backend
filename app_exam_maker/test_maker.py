...
def fetch_boards(request: Request) -> BoardsResponse:
    ...
    boards = get_boards(conn=context.conn, user_code=user_code, user_type=user_type)
    return BoardsResponse(message="Boards fetched successfully", boards=boards)
...
def fetch_classes_by_board(board_id: int, request: Request) -> ClassesResponse:
    context, user_code, user_type = get_context_with_user_info(request)
    classes = get_classes_against_board(conn=context.conn, board_id=board_id, user_code=user_code, user_type=user_type)
    return ClassesResponse(message="Classes fetched successfully", classes=classes)
...
def fetch_subjects_by_class(class_id: int, request: Request) -> SubjectsResponse:
    context, user_code, user_type = get_context_with_user_info(request)
    subjects = get_subjects_against_class_board(conn=context.conn, class_id=class_id, user_code=user_code, user_type=user_type)
    return SubjectsResponse(message="Subjects fetched successfully", subjects=subjects)
...
def fetch_topics_by_subject(subject_id: int, request: Request) -> TopicsResponse:
    context, user_code, user_type = get_context_with_user_info(request)
    topics = get_topics_against_subject(conn=context.conn, subject_id=subject_id, user_code=user_code, user_type=user_type)
    return TopicsResponse(message="Topics fetched successfully", data=topics)
...
def fetch_chapters_by_subject(subject_id: int, request: Request) -> ChaptersResponse:
    context, user_code, user_type = get_context_with_user_info(request)
    return get_chapters_against_subject(conn=context.conn, subject_id=subject_id, user_code=user_code, user_type=user_type)
